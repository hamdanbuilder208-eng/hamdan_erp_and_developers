...
export default function CustomerReceiptPrintPage() {
  ...
  const { data: receipt, isLoading } = useQuery({
    ...
    queryFn: async () => (await portalApi.get<Receipt>(`/portal/me/receipts/${id}`)).data,
  });

  const { data: companySettings } = useQuery({
    queryKey: ["portal-company-settings"],
    queryFn: async () => (await portalApi.get<CompanySettings>("/portal/company-settings")).data,
  });

  const { data: allMyReceipts } = useQuery({
  ...
  const orderedHistory = (allMyReceipts ?? [])
    .filter((r) => r.booking_id === receipt.booking_id)
    .slice()

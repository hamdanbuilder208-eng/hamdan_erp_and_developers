...
export default function CustomerSignupPage() {
  ...
  const signup = useMutation({
    mutationFn: async () =>
      (
        await portalApi.post<CustomerAccount>("/portal/signup", {
          mobile: form.mobile,
          cnic: form.cnic,
    ...
    onSuccess: () => {
      navigate("/portal/login", { state: { justSignedUp: true } });
    },
    onError: (err: unknown) => {
  ...
  return (
    <div className="flex min-h-screen w-full items-center justify-center bg-surface px-6 py-12">
      <div className="w-full max-w-sm">
        ...
        <h1 className="text-2xl font-semibold text-navy-950 dark:text-white">Create your account</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Enter the mobile number and CNIC on file with your booking to verify your identity.
        </p>
